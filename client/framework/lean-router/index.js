...
// hub._routeParser = path => {
//     let req = {};
//     let [uri, queryString] = path.split('?');
//     let prefix = null;
//     if(hub.location){
//         prefix = hub.util.compareUrl(hub.location, uri);
//     }
//     let uriParts = uri.split('/');
//     req.params = {};
//     req.paramList = [];
...
//         if(hub.location === '/' + uri){
//             //sync state
//             hub.trigger('sync-state-query', req.query)
//             return null;
//         }
//     }
...
//                 return i;
//             }
//             return hub.util.combineUriParts(uriParts, index, i);
//         });
//     }
//     if(prefix){
//         req.hints = req.hints.filter(hint=>hint.length > prefix.length);
...
// hub._parseRoute = () => riot.route.parser(hub._routeParser);

// hub.search = (param, value) => hub.trigger('state-search', {param, value});

// hub.util = {
//     completePart: uri => uri.startsWith('/') ? uri : ('/' + uri),
//     compareUrl: (u1, u2) => {
//         var r = [];
//         var arr1 = u1.split('/');
//         var arr2 = u2.split('/');
...
//         return r.join('/')
//     },
//     combineUriParts: (parts, i, combined) => {
//         if(!parts.length || i<=0){
//             return combined;
//         }
//         let uri = parts[i-1] + '/' + combined;
//         return hub.util.combineUriParts(parts, --i, uri);
//     },
//     composeObject: (ks, vs) => {
//         var o = {};
//         if(!Array.isArray(ks) || !Array.isArray(vs) || ks.length != vs.length){
...
//     extractParams: path => path.match(/_[a-zA-Z0-9:]+/g),

//     toPattern: route => route.replace(/_[a-zA-Z0-9:]+/g, "*")
// }

// hub.registerRoute = ({path, name, resolve, redirectTo, ...rest}, container) => {
//     hub.routes[path] = {
//         resolve,
//         redirectTo,
//         tag: container.tags[name],
...
//         }
//         if((!tag.hasOwnProperty('show') || tag.show) 
//             && hub.util.completePart(path) === hub.location){
//             return recursiveHints(hints.slice(1), ctx);
//         }
...
//                     $location,
//                     ctx,
//                     hub._executeMiddlewares(tag, tag.$mws, ctx, pendingDone),
//                 );
//             });
//             function pendingDone(){
//                 requestAnimationFrame(()=>{
//                     hub.trigger('history-resolve', hub._getMetaDataFromRouteMap(hub.location).route, route, ctx, hints, ()=>{
...
// hub._executeMiddlewares = (component, mws, ctx, done) => {
//     return function nextFn(){
//         if(!mws || !mws.length){
//             return done();
//         }
//         mws[0].call(component, () => hub._executeMiddlewares(component, mws.slice(1), ctx, done)(), ctx);
//     }
// };
...
//  * Exchange control flow to hub from riot router
//  */
// hub.go = url => riot.route(url);

// hub._getMetaDataFromRouteMap = routeKey => {
//     routeKey = routeKey && routeKey.startsWith('/') ? routeKey : '/' + routeKey;
//     let keys = Object.keys(hub.routes);
//     for(let i=0, len=keys.length; i<len; i++){
//         let k = keys[i];
//         let route = hub.routes[k];
//         if(hub.util.toPattern(k) === hub.util.toPattern(routeKey)){
//             let paramKeys = (hub.util.extractParams(k) || []).map(i=>i.slice(2));
//             let paramValues = (hub.util.extractParams(routeKey) || []).map(i=>i.slice(1));
//             return {
//                 route,
//                 $state: k,
//                 $location: routeKey,
//                 params: hub.util.composeObject(paramKeys, paramValues)
//             };
//         }
//     }
//     return {
